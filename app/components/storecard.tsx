...
const StoreCard: React.FC<Props> = ({ store, isList, expanded, cardWidthStore }) => {
    const renderCountRef = useRef(0);
    renderCountRef.current += 1;

    console.log(
        `StoreCard render -> id: ${store.id}, expanded: ${expanded}, renderCount: ${renderCountRef.current}`
    );
    ...
    return (
        ...
        >
            <View className={`${!isList ? 'flex flex-row ' : ''}`}>
                <TouchableOpacity onPress={() => { }} className="relative mr-2">
                    <Image
