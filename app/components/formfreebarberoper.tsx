...
export const FormFreeBarberOperation = ({ freeBarberId, enabled }: { freeBarberId: string; enabled: boolean }) => {
    ...
    const [triggerGetFreeBarberPanel, { data, isError }] = useLazyGetFreeBarberMinePanelDetailQuery();
    ...
    const mapTypeToLabel = (t: number): string => {
        switch (t) {
            case 0: return 'MaleHairdresser';
            case 1: return 'FemaleHairdresser';
            default: return ''; // Veya varsayılan bir değer
        }
    };

    const effectiveType = selectedType
        ? selectedType
        : (data?.type != null ? mapTypeToLabel(data.type) : undefined);
    ...
    useEffect(() => {
        ...
        if (data) {
            ...
            const initialType = mapTypeToLabel(data.type);
            reset({
                ...getValues(),
                name: data?.firstName,
