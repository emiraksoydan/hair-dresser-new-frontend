...
export default function StoreDetail() {
    ...
    // store header info
    const { data: storeData } = useGetStoreForUsersQuery(strId, { skip: !storeId });
    const { data: workingHours } = useGetWorkingHoursByTargetQuery(strId, { skip: !storeId });
                ...
                <View className="p-4 z-0 gap-3">
                    ...
                    {isFreeBarber && (
                        <View className="bg-gray-800 rounded-xl p-3">
                            <Text className="text-white">
                                {isHourlyFree
                                    ? `Saatlik Kiralama: ${pricingValue} TL / saat`
                                    : isPercentFree
                                        ? `Komisyon: %${pricingValue}`
                                        : `Ücret Tipi: ${String(storeData?.pricingType ?? "-")}`}
                            </Text>
                        </View>
                    )}
                    <Text className="text-white font-ibm-plex-sans-regular text-xl mt-2">
                        {isHourlyFree ? `Saatlik Kiralama: (${totalPrice} TL)` : `İşlemler : (${totalPrice} TL)`}
                    </Text>
                    {!isHourlyFree && (
                        <FlatList
                    ...
                    <TouchableOpacity
                        disabled={!canSubmit}
                        className={`py-3 flex-row justify-center gap-2 rounded-xl mt-1 items-center ${(!canSubmit) ? "bg-[#4b5563] opacity-60" : "bg-[#22c55e] opacity-100"}`}
                        onPress={async () => {
                            try {
